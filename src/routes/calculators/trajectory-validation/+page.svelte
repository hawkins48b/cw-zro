<script>
	...
	let chartData = $derived.by(() => {
		...
		const step = Math.max(1, Math.round(rangeDist / 40));
		const chartRange = { distance: rangeDist, unit: trajectoryValidation.range.unit, step };

		const initial = calculateFullTrajectory(profile, chartRange, null, null);
		...
		const validated = calculateFullTrajectory(validatedProfile, chartRange, null, null);

		if (!initial || !validated) return null;
	...
	function buildChartOptions(data, dark, metric, mType, mUnit, rangeVal) {
		...
		const initialData = (data.initial.chartPoints ?? []).map((p) => ({
			x: Math.round(p.distance.In(distUnit) * 10) / 10,
			y: getYValue(p)
		}));
		const validatedData = (data.validated.chartPoints ?? []).map((p) => ({
			x: Math.round(p.distance.In(distUnit) * 10) / 10,
			y: getYValue(p)
		}));
		...
		return {
			series: [
				{ name: m.traj_val_initial(), data: initialData },
				{ name: m.traj_val_validated(), data: validatedData }
			],
			chart: {
				type: 'line',
				height: 280,
			...
			stroke: { width: [2, 2], curve: 'monotoneCubic', dashArray: [4, 0] },
